...
if msg.query and msg.query:sub(1,6) == "Menu:-" and msg.query:gsub("Menu:-",""):match('%d+') and is_sudo(msg) then
	...
	keyboard.inline_keyboard = {
		{
            {text = "❤️ "..tostring(redis:get(RedisIndex.."TiiGeRLikes")), callback_data="/like:"..chatid},
            {text = "💔 "..tostring(redis:get(RedisIndex.."TiiGeRDisLikes")), callback_data="/dislike:"..chatid}
        },
		{
...
if msg.cb then
  ...
  if matches[1] == '/like' then
      if redis:get(RedisIndex.."IsDisLiked:"..msg.from.id) then
        ...
        local dislikes = redis:get(RedisIndex.."TiiGeRDisLikes")
        redis:set(RedisIndex.."TiiGeRDisLikes",dislikes - 1)
        redis:set(RedisIndex.."IsLiked:"..msg.from.id,true)
        local likes = redis:get(RedisIndex.."TiiGeRLikes")
        redis:set(RedisIndex.."TiiGeRLikes",likes + 1)
